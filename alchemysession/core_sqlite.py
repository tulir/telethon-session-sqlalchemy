...
class AlchemySQLiteCoreSession(AlchemyCoreSession):
    def set_update_state(self, entity_id: int, row: Any) -> None:
        self.engine.execute(f"INSERT OR REPLACE INTO {self.UpdateState.__tablename__} "
                            f"VALUES (:session_id, :entity_id, :pts, :qts, :date, :seq, "
                            f"        :unread_count)",
    ...
    def process_entities(self, tlo: Any) -> None:
        ...
        self.engine.execute(f"INSERT OR REPLACE INTO {self.Entity.__tablename__} "
                            f"VALUES (:session_id, :id, :hash, :username, :phone, :name)",
    ...
    def cache_file(self, md5_digest: str, file_size: int,
        ...
        self.engine.execute(f"INSERT OR REPLACE INTO {self.SentFile.__tablename__} "
                            f"VALUES (:session_id, :md5_digest, :type, :file_size)",
